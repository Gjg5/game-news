...
# 非新闻类排除（允许游戏相关但排除列表/指南/评测类内容）
NON_NEWS_KW = ["最佳", "最好", "所有", "如何", "指南", "guide", "how to", "技巧",
               "攻略", "推荐", "排行", "top", "best", "list", "汇总", "整理",
               "值得买", "优惠", "史低", "折扣", "降价"]
...
GAME_KW = ["游戏","gam","play","电竞","PS5","Xbox","Switch","Steam","任天堂","索尼","微软",
           ...
           "角色","武器","地图","皮肤","战斗","动作","冒险",
           "RPG","FPS","ARPG","MMO","MOBA","格斗","射击","竞速","模拟",
           "Steam Deck","Switch 2","PS4","PS5 Pro","Xbox Series",
           "开服","关服","公测","封测","停运","销量","收入","财报",
           "曝光","公布","官宣","预告片","实机","演示","试玩"]

def is_gaming(text):
    """判断是否为可用的游戏新闻（排除非游戏、列表/指南类内容）"""
    t = (text or "").lower()
    for kw in NON_GAME_KW:
        if kw.lower() in t:
            return False
    # 排除列表/指南/评测等非新闻类内容
    for kw in NON_NEWS_KW:
        if kw.lower() in t:
            return False
    for kw in GAME_KW:
        if kw.lower() in t:
